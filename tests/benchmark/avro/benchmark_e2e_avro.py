...
from confluent_kafka import Producer, Consumer, Schema
...
def benchmark_single_subject_produce(conf, topic):
    ...
    producer = AvroProducer(conf, default_value_schema=basic, default_key_schema=basic)
    profile.enable()
...
def benchmark_generic_serde_produce(conf, topic):
    ...
    serializer = AvroSerializer(SchemaRegistryClient(url=sr_conf.get('url')), schema=basic)
...
def benchmark_serde_batch_consume(conf, topic):
    ...
    serializer = AvroSerializer(SchemaRegistryClient(**sr_conf), schema=basic)
