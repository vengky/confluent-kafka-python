...
class TrivupFixture(object):
    ...
    def producer(self, options=None):
        ...
        :param dict options: additional producer configuration values to be used.
    ...
    def consumer(self, options=None):
        ...
        """
        if options is None or 'group.id' not in options:
            options.update({'group.id': uuid1()})

        return Consumer(self.client_conf(options))
